...
def colaborativeFiltering_ItemBased_new(user_ratings, userID="672"):
    # the ids returned are from the movielens dataset
    recommendatons_movieLens = runnewItemBasedColaborativeFiltering(testSubject=str(userID))
    print(recommendatons_movieLens)
    recommendations_tmdb = []
    for recommendation in recommendatons_movieLens:
        recommendations_tmdb.append(get_tmdbID_from_movielensTD(int(recommendation)))
...
def colaborativeFiltering_ItemBased_nmf(user_ratings, userID="672"):
    # the ids returned are from the movielens dataset
    recommendatons_movieLens = runnmfItemBasedColaborativeFiltering(testSubject=str(userID))
    print(recommendatons_movieLens)
    recommendations_tmdb = []
    for recommendation in recommendatons_movieLens:
        recommendations_tmdb.append(get_tmdbID_from_movielensTD(int(recommendation)))
...
def item_colaborativefiltering():
    ...
    userID = add_user_to_dataset(userID, user_ratings)
    recommendations = colaborativeFiltering_ItemBased(user_ratings, 100)
    print(recommendations)
    reset_files()
    return jsonify(recommendations)
...
def newitem_colaborativefiltering():
    ...
    print("user Rating in tuple format = ", user_ratings)
    userID = add_user_to_dataset(userID, user_ratings)
    recommendations = colaborativeFiltering_ItemBased_new(user_ratings,300)
    print(recommendations)
    reset_files()
    return jsonify(recommendations)
...
def nmfitem_colaborativefiltering():
    ...
    print("user Rating in tuple format = ", user_ratings)
    userID = add_user_to_dataset(userID, user_ratings)
    recommendations = colaborativeFiltering_ItemBased_nmf(user_ratings,300)
    print(recommendations)
    reset_files()
    return jsonify(recommendations)
